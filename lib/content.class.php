	...
	/*
      ...
      public function renderMenu($array, $parent_id = 0, $menuid = 'main-menu', $class = 'top-menu')
      {
		  ...
		  if(is_array($array) && count($array) > 0) {
			  ...
			  $pageslug = App::Core()->pageslug;
			  $modslug = App::Core()->slugs->module;
				  ...
				  if ($row['parent_id'] == $parent_id) {
					  ...
					  switch ($row['content_type']) {
						  case 'module':
							  ...
							  $murl = SITEURL . '/' . $modslug->{$row['mslug']} . '/';
							  $murl2 = $row['home_page'] ? SITEURL.'/' : $murl;
							  $link = '<a data-mslug="' . $modslug->{$row['mslug']} . '" href="' . $murl2 . '" class="' . $mactive . '">' . $icon . $name . $caption . '</a>';
							  break;
							  
						  case 'page':
						      $active = (in_array($row['pslug'], $segments) ? "active" : "normal");
							  if ($row['home_page']) {
