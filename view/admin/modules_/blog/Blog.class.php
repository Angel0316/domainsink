<?php
   ...
   * @copyright 2016
   * @version $Id: Blog.class.php, v5.00 2016-07-07 18:12:05 gewa Exp $
   */
  ...
  class Blog
  {
      ...
      public function AdminIndex()
      {
          ...
          if (isset($_GET['letter']) and (isset($_POST['fromdate']) && $_POST['fromdate'] <> "")) {
              ...
              $counter = Db::run()->count(false, false, "SELECT COUNT(*) FROM `" . self::mTable . "` WHERE `created` BETWEEN '" . trim($fromdate) . "' AND '" . trim($enddate) . " 23:59:59' AND `title" . Lang::$lang . "` REGEXP '^" . $letter . "'");
              $where = "WHERE `created` BETWEEN '" . trim($fromdate) . "' AND '" . trim($enddate) . " 23:59:59' AND `d.title" . Lang::$lang . "` REGEXP '^" . $letter . "'";

          } elseif (isset($_POST['fromdate']) && $_POST['fromdate'] <> "") {
              $counter = Db::run()->count(false, false, "SELECT COUNT(*) FROM `" . self::mTable . "` WHERE `created` BETWEEN '" . trim($fromdate) . "' AND '" . trim($enddate) . " 23:59:59'");
