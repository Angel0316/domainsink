<?php
   ...
   * @copyright 2018
   * @version $Id: plugins.tpl.php, v1.00 2018-05-05 10:12:05 gewa Exp $
   */
  if (!defined("_WOJO"))
...
<div class="row half-gutters align-middle">
  ...
  <div class="columns content-right mobile-50 mobile-content-left mobile-order-2">
    <a href="<?php echo Url::url(Router::$path, "new/");?>" class="wojo secondary button"><i class="icon plus alt"></i><?php echo Lang::$word->PLG_SUB1;?></a>
  </div>
</div>
<div class="half-top-padding"><?php echo Validator::alphaBits(Url::url(Router::$path), "letter", "wojo small bold text horizontal link divided list align-center");?>
</div>
...
<div class="row phone-block-1 mobile-block-1 tablet-block-2 screen-block-3 gutters content-center">
  ...
  <div class="column" id="item_<?php echo $row->id;?>">
    <div class="wojo attached segment content-center">
      <img src="<?php echo $row->icon ? APLUGINURL . $row->icon : SITEURL . '/assets/images/basic_plugin.svg';?>" class="wojo normal basic image" alt="">
      ...
      <div class="content-left">
        <div class="row no-all-gutters">
          <div class="columns">
            ...
            <a data-set='{"option":[{"<?php echo $row->plugalias ? "delete" : "trash";?>": "<?php echo $row->plugalias ? "deletePlugin" : "trashPlugin";?>","title": "<?php echo Validator::sanitize($row->{'title' . Lang::$lang}, "chars");?>","id":<?php echo $row->id;?>}],"action":"<?php echo $row->plugalias ? "delete" : "trash";?>","parent":"#item_<?php echo $row->id;?>"}' class="wojo icon negative circular button action">
            <i class="icon trash"></i>
            </a>
          </div>
          <?php if($row->hasconfig):?>
          <div class="columns shrink">
            <a href="<?php echo Url::url(Router::$path, $row->plugalias);?>" class="wojo icon circular button"><i class="icon cogs"></i></a>
          </div>
          <?php endif;?>
        </div>
      </div>
