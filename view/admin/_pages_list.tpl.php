<?php
   ...
   * @copyright 2018
   * @version $Id: _pages_list.tpl.php, v1.00 2018-05-05 10:12:05 gewa Exp $
   */
  if (!defined("_WOJO"))
...
<div class="row half-gutters align-middle">
  ...
  <div class="columns content-right mobile-50 mobile-content-left mobile-order-2">
    <a href="<?php echo Url::url(Router::$path, "new/");?>" class="wojo secondary button"><i class="icon plus alt"></i><?php echo Lang::$word->PAG_SUB4;?></a>
  </div>
</div>
<?php if(!$this->data):?>
...
<div class="wojo segment">
  <table class="wojo sorting basic table">
    ...
    <tr id="item_<?php echo $row->id;?>">
      <td class="collapsing"><span class="wojo tiny simple label"><?php echo $row->id;?></span></td>
      ...
      <td class="collapsing center aligned"><?php if($row->page_type == "contact"):?>
        ...
        <?php endif;?></td>
      <td class="collapsing"><a href="<?php echo Url::url(Router::$path, "edit/" . $row->id);?>" class="wojo icon positive button"><i class="icon note"></i></a>
        <?php if($row->page_type == "normal"):?>
        <a data-set='{"option":[{"trash": "trashPage","title": "<?php echo Validator::sanitize($row->{'title' . Lang::$lang}, "chars");?>","id":<?php echo $row->id;?>}],"action":"trash","parent":"#item_<?php echo $row->id;?>"}' class="wojo icon simple button action">
          <i class="icon negative trash"></i>
        </a>
        <?php else:?>
        <a class="wojo icon circular basic disabled button">
...
<div class="row half-gutters-mobile half-gutters-phone align-middle">
  <div class="columns shrink mobile-100 phone-100">
    <div class="wojo small light text"><?php echo Lang::$word->TOTAL.': '.$this->pager->items_total;?> / <?php echo Lang::$word->CURPAGE.': '.$this->pager->current_page.' '.Lang::$word->OF.' '.$this->pager->num_pages;?></div>
  </div>
