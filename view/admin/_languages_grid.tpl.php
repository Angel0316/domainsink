<?php
   ...
   * @copyright 2018
   * @version $Id: _languages_grid.tpl.php, v1.00 2018-01-08 10:12:05 gewa Exp $
   */
  if (!defined("_WOJO"))
...
<div class="row half-gutters align-middle">
  ...
  <div class="columns shrink mobile-100"> <a href="<?php echo Url::url(Router::$path, "new/");?>" class="wojo secondary button"><i class="icon plus alt"></i><?php echo Lang::$word->LG_SUB5;?></a> </div>
</div>
    ...
    <div class="wojo segment"> <img src="<?php echo ADMINVIEW;?>/images/langbg.jpg">
      ...
      <div class="basic footer">
        <div class="row align-middle half-vertical-gutters no-gutters">
          <div class="columns">
            <div class="wojo simple buttons"> <a href="<?php echo Url::url(Router::$path . "/edit", $row->id);?>" class="wojo button"> <span class="flag icon <?php echo $row->abbr;?>"></span> <?php echo $row->name;?></a> <a href="<?php echo Url::url(Router::$path . "/translate", $row->id);?>" class="wojo icon button"> <i class="icon positive bubble"></i> </a> <a data-set='{"option":[{"delete": "deleteLanguage","title": "<?php echo Validator::sanitize($row->name, "chars");?>","id":<?php echo $row->id;?>}],"action":"delete","parent":"#item_<?php echo $row->id;?>"}' class="wojo icon button action"> <i class="icon negative trash"></i> </a> </div>
          </div>
