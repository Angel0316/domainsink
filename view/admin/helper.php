<?php
   ...
   * @copyright 2018
   * @version $Id: helper.php, v1.00 2018-12-05 10:12:05 gewa Exp $
   */
  define("_WOJO", true);
              ...
              if (strlen($string) > 3):
                  ...
                  if ($result = Db::run()->pdoQuery($sql)->results()):
                      ...
                      foreach ($result as $row):
						  ...
						  if($row->hasconfig):
							  $html .= '<td class="wojo large text">';
							  $html .= '<a href="' . Url::url("admin/plugins", $row->plugalias) . '" class="wojo icon basic primary circular small button"><i class="icon cogs"></i></a>';
							  $html .= '</td>';
						  endif;
                          $html .= '</tr>';
                      endforeach;
  ...
  if (isset($_GET['doAction'])):
      switch ($_GET['page']) :
          ...
          /* == Get Language File == */
          case "languagefile":
			  if (File::exists(BASEPATH . Lang::langdir . Core::$language . "/" . $_GET['section'])):
				  $xmlel = simplexml_load_file(BASEPATH . Lang::langdir . Core::$language . "/" . $_GET['section']);
				  $tpl = App::View(BASEPATH . 'view/admin/snippets/');
				  $tpl->xmlel = $xmlel;
				  $tpl->section = null;
  ...
  if (isset($_POST['quickedit'])):
      ...
      switch ($_POST['type']) :
          ...
          case "phrase":
			  if (file_exists(BASEPATH . Lang::langdir . Core::$language . "/" . $_POST['path'])):
				  $xmlel = simplexml_load_file(BASEPATH . Lang::langdir . Core::$language . "/" . $_POST['path']);
				  $node = $xmlel->xpath("/language/phrase[@data = '" . $_POST['key'] . "']");
				  $node[0][0] = $title;
				  $xmlel->asXML(BASEPATH . Lang::langdir . Core::$language . "/" . $_POST['path']);
			  endif;
              break;  
      endswitch;
	  $json['title'] = $title;
